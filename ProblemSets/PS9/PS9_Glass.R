...
# Run the recipe
housing_prep <- housing_recipe %>% prep(data = housing_train, retain = TRUE)
...
# Calculate in-sample RMSE for the Lasso model
in_sample_rmse_lasso <- lasso_fit %>%
  predict(housing_train_prepped) %>%
  mutate(truth = housing_train_prepped$medv) %>%
  rmse(truth, .pred)

in_sample_rmse_lasso
